...
class ExtractorEvaluator(object):
    def __init__(self, reg_val, batchsize, ckp_path, data_path, roomid):
        ...
        for m in self.metric_list:
            self._name.append(m[0])
        self._name.append('loss')
        self.train_record, self.test_record = dict(), dict()
        for i in range(len(self._name)):
            self.train_record[self._name[i]] = []
        ...
        self.ind1 = [torch.tensor(0, dtype=torch.int64)] + self.ind2[:-1]
        self.extractor = self.extract()
        self.extractor.send(None)
        ...
        while True:
            ...
            for batch_idx, data in enumerate(self.train_dataloader):
                ...
                for i, val in enumerate(avg_metrics):
                    self.train_record[self._name[i]].append(val)
                ...
                yield vec_grads, self.train_record
    ...
    def evaluate(self):
        with torch.no_grad():
            ...
            for batch_idx, data in enumerate(self.test_dataloader):
                ...
                for i, val in enumerate(avg_metrics):
                    self.test_record[self._name[i]].append(val)
